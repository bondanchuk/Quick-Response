...
class Terselesaikan extends CI_Controller {
  ...
  public function __construct(){
    ...
    $this->load->model('Admin/Selesai','tbl_mhs');
...
//    }
    ...
    $this->load->view('Admin/OperatorSelesai');
...
if($tbl_mhs->status=="Terselesaikan"){

              $row[] = '<a class="btn btn-circle btn-success"><i class="fa fa-check "></i></a>';
}else if($tbl_mhs->status=="Pending"){
$row[] = '<a class="btn btn-circle btn-warning"><i class="fa fa-hourglass"></i></a>';
}
 ...
 <a class="btn btn-sm btn-danger" href="#map-canvas" title="Edit" onclick="viewmarkerjembatan('."'".$tbl_mhs->idLaporan."'".')"><i class="fa fa-map-marker "></i> View Lokasi</a>';
    ...
    $this->db->delete('login_user');
...

      {
        ...
        $data = array('status' => $this->input->post('updateStatus')



        );

        $this->tbl_mhs->updateStatus($id, $data);
